...
// ===== API PERMISSION MAP =====
// Defines which roles can access which API endpoints (by route pattern and method)
export const API_PERMISSIONS = {
  // Bootstrap - all authenticated users need this
  '/bootstrap': {
...
// Helper to check if a user has permission for a given API route + method
export function hasApiAccess(role, route, method) {
  // Remove leading /api if present
  const normalizedRoute = route.startsWith('/api') ? route.slice(4) : route;
  // Find matching permission (exact match or prefix)
